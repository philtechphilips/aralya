...
const FAQSection = ({ title, description, faqs }: FAQSectionProps) => {
  ...
  return (
    <section ref={sectionRef} className="w-full md:px-10 px-5 py-25 bg-white">
      <h2 className={`text-[#0E1C29] md:text-[56px] text-4xl font-normal text-center transition-all duration-400 ease-out ${
        isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'
      }`}>
        {title}
      </h2>
      <p className={`mt-4 text-[#0E1C29] text-base text-center font-semibold transition-all duration-400 ease-out delay-150 ${
        isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'
      }`}>
        {description}
      </p>
          ...
          return (
            ...
            >
              <div className="mb-2 w-full flex items-center justify-between">
                ...
                <i className={`text-xl mt-0.25 transition-all duration-200 ease-in-out ${
                  isOpen ? 'text-white rotate-180' : 'text-[#0E1C29] rotate-0'
                } ri-arrow-down-s-line`}></i>
              </div>
